/*************************************************************************************************
**
**  Description:  ULTIMATE COMBINED SCRIPT - AUDIT VERSION
**                Generates a complete and exhaustive T-SQL script for all permissions,
**                including those on system principals like dbo, guest, and INFORMATION_SCHEMA.
**                Use with caution for migrations.
**
**  Version:      ULTIMATE - V12 (Audit Version)
**
...
DECLARE login_cursor CURSOR FOR
    ...
    WHERE p.type IN ('S', 'U', 'G') AND p.principal_id > 1 AND p.name NOT LIKE '##%';
...
BEGIN
    ...
    -- DB USERS (Now includes ALL principals)
    PRINT '-- [-- DB USERS --] --';
    SET @sql = N'USE ' + QUOTENAME(@db_name) + N';
        SELECT
            CASE
                WHEN rm.type = ''E'' THEN ''IF NOT EXISTS (SELECT [name] FROM sys.database_principals WHERE [name] = '''''' + rm.name + '''''') BEGIN CREATE USER '' + QUOTENAME(rm.name) + '' FROM EXTERNAL PROVIDER; END''
                WHEN rm.authentication_type IN (2, 0) AND (SELECT SUBSTRING(convert(sysname, SERVERPROPERTY(''productversion'')), 1, charindex(''.'',convert(sysname, SERVERPROPERTY(''productversion'')))-1)) > 10 THEN ''IF NOT EXISTS (SELECT [name] FROM sys.database_principals WHERE [name] = '''''' + rm.name + '''''') BEGIN CREATE USER '' + QUOTENAME(rm.name) + '' WITHOUT LOGIN WITH DEFAULT_SCHEMA = '' + QUOTENAME(rm.default_schema_name) + '', SID = '' + CONVERT(varchar(1000), rm.sid) + '' END;''
        ...
        WHERE rm.type IN (''X'',''S'',''G'',''U'',''E''); -- Includes ALL user/group principal types
        IF @@ROWCOUNT = 0 PRINT ''-- (No users found)'';';
    EXEC sp_executesql @sql;

    -- ORPHANED USERS (Logic remains specific to user-created accounts)
    ...
    -- APPLICATION ROLES (Now includes all)
    PRINT '-- [-- APPLICATION ROLES --] --';
    SET @sql = N'USE ' + QUOTENAME(@db_name) + N';
        IF EXISTS(SELECT 1 FROM sys.database_principals WHERE type = ''A'')
            PRINT ''-- NOTE: Application role passwords cannot be scripted. A placeholder password is used.'';
        SELECT ''CREATE APPLICATION ROLE '' + QUOTENAME(name COLLATE DATABASE_DEFAULT) + '' WITH DEFAULT_SCHEMA = '' + QUOTENAME(ISNULL(default_schema_name COLLATE DATABASE_DEFAULT, ''dbo'')) + '', PASSWORD = '' + QUOTENAME(''replace_this_password_immediately'', '''''''') + '';'' + CHAR(13) + CHAR(10) + ''GO''
        FROM sys.database_principals WHERE type = ''A'';
        IF @@ROWCOUNT = 0 PRINT ''-- (No application roles found)'';';
    EXEC sp_executesql @sql;

    -- DB ROLES (Now includes system principals)
    PRINT '-- [-- DB ROLES --] --';
    SET @sql = N'USE ' + QUOTENAME(@db_name) + N';
    ...
    -- OBJECT AND COLUMN LEVEL PERMISSIONS (Now includes system principals)
    ...
    -- TYPE LEVEL PERMISSIONS (Now includes system principals)
    ...
    SET @sql = N'USE ' + QUOTENAME(@db_name) + N';
        ...
        WHERE perm.class_desc = ''TYPE'';
        IF @@ROWCOUNT = 0 PRINT ''-- (No type level permissions found)'';';
    EXEC sp_executesql @sql;

    -- DB LEVEL PERMISSIONS (Now includes system principals)
    ...
    -- SCHEMA PERMISSIONS (Now includes system principals)
