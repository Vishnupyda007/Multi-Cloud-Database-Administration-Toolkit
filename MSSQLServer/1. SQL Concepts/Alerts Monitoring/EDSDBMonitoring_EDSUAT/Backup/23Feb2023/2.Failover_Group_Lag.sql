...
declare @replicationlag int =2
IF OBJECT_ID(N'tempdb..#Managed_Instance_Failover_Group_Lag') IS NOT NULL
BEGIN
...
BEGIN
	...
	SELECT 'Your have established a connection to a Secondary Replica of SQL MI, where the information about Failover Groups is not available!' as Repli_endpoint_url,''Last_Hardened_Lsn, ''Last_Redone_Lsn,''Redo_Queue_Size,''Catchup_Progress,
		''End_of_log_lsn,''internal_state_desc,''database_state_desc,''partner_database,''ReplicationLag,''displaySeverity;
END
ELSE 
BEGIN
	...
	BEGIN
							...
							if (select count(1) from [#Managed_Instance_Failover_Group_Lag] where Repli_endpoint_url not like '%tcp://%')>0
							begin
							insert into [#Managed_Instance_Failover_Group_Lag]
							select * from [#Managed_Instance_Failover_Group_Lag] where  ReplicationLag>@replicationlag OR (ReplicationLag IS NULL and displaySeverity = 'ERROR')
							end
							else
							begin
							insert into [#Managed_Instance_Failover_Group_Lag]
							SELECT 'This SQL MI is NOT PRIMARY!' as Repli_endpoint_url,''Last_Hardened_Lsn, ''Last_Redone_Lsn,''Redo_Queue_Size,''Catchup_Progress,
							''End_of_log_lsn,''internal_state_desc,''database_state_desc,''partner_database,''ReplicationLag,''displaySeverity;
							end
	...
	ELSE
		insert into [#Managed_Instance_Failover_Group_Lag]
		SELECT 'Your SQL MI is NOT using Failover Groups!' as Repli_endpoint_url,''Last_Hardened_Lsn, ''Last_Redone_Lsn,''Redo_Queue_Size,''Catchup_Progress,
		''End_of_log_lsn,''internal_state_desc,''database_state_desc,''partner_database,''ReplicationLag,''displaySeverity;

END

select *,1 as [Table_Status],getdate() as Date_Collection from [#Managed_Instance_Failover_Group_Lag] where
ReplicationLag>@replicationlag OR (ReplicationLag IS NULL and displaySeverity = 'ERROR')