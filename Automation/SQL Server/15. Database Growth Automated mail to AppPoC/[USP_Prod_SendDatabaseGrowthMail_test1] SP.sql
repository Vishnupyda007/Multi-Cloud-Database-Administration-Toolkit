...
/****** Object:  StoredProcedure [dbo].[USP_Prod_SendDatabaseGrowthMail_test1]    Script Date: 7/21/2025 5:26:12 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[USP_Prod_SendDatabaseGrowthMail_test1] --consideration main one 
with Encryption
AS  
BEGIN  
    SET NOCOUNT ON;  
    ...
    WHERE gd.TotalDataFileSizeGB_PercentChange >= 0 -- Only process AppPoCs with non-negative growth  
    ORDER BY ai.AppPoC;  
    ...
    BEGIN  
        ...
        SET @EmailBody =  
            ...
            '<p>Hi ' + @AppPoC + ',</p>' +  
            '<p>This is to inform you that the following database(s) associated with your application <strong>' + @AppName + '</strong> have experienced a growth of more than 10% in Production compared to the ' + FORMAT(CAST('2025-05-27' AS DATE), 'dd MMM
M yyyy') + ' baseline. Kindly review the details below and check the possibility to purge the unwanted data:</p>' +  
            ...
            (  
                SELECT  
                    ...
                    (  
                        ...
                        WHERE  
                            ai.AppPoC = @AppPoC  
                            AND gd.TotalDataFileSizeGB_PercentChange >= 0  
                        FOR XML PATH(''), TYPE  
                    ).value('.', 'NVARCHAR(MAX)') +  
                    '</table>'  
                    +  
         ...
         WHERE ServerName = tt.ServerName AND DatabaseName = tt.DatabaseName)  
                        ...
                        WHERE gd_sub.ServerName = gd.ServerName  
                          ...
                          AND gd_sub.TotalDataFileSizeGB_PercentChange >= 0  
                        GROUP BY gd_sub.ServerName, gd_sub.DatabaseName  
                        FOR XML PATH(''), TYPE  
                    ).value('.', 'NVARCHAR(MAX)')  
                FROM #GrowthData gd  
                JOIN ApplicationInventory ai  
                ...
                WHERE  
                    ai.AppPoC = @AppPoC  
                    AND gd.TotalDataFileSizeGB_PercentChange >= 0  
                GROUP BY gd.ServerName, gd.DatabaseName  
            ...
            + '<br/><br/><p style="font-family:Serif;">Thanks &amp; Regards,<br/><b>ITOps 1C DBA Team<b></p>'; -- Wrapped in <p> with inline style  
