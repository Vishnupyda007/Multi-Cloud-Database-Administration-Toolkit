...
BEGIN
    CREATE TABLE #temp_SIT_DB_Weekly_Comparison (
        ...
        TotalDatabaseSizeGB_OneWeekAgo NVARCHAR(50),
        TotalDBSizeChange NVARCHAR(50),
    ...
    SELECT DISTINCT
        ...
        FileId,
        TotalDatabaseSizeGB,
        TotalDatabaseSizeGB_OneWeekAgo,
        TotalDBSizeChange,
        ...
        FormattedDate,
        CASE 
            WHEN CAST(TotalDBSizeChange AS FLOAT) > 0 THEN 'Increased'
            WHEN CAST(TotalDBSizeChange AS FLOAT) < 0 THEN 'Decreased'
            ELSE 'No Change'
...
and a.Datetime >= CAST(GETDATE() AS DATE)
    ...
    SELECT @body_HTML = @body_HTML + 
        ...
        <th>Database Total SizeGB(OneWeekAgo)</th>
        <th>Total DBSize Change(GB)</th>
        ...
        (SELECT STUFF((
            SELECT
              '<tr>
                ...
                <td>' + TotalDatabaseSizeGB_OneWeekAgo + '</td>
                <td>' + TotalDBSizeChange + '</td>
                <td>' + PercentageOfTotalGBUsed + '</td> 
                <td>' + PercentageOfTotalGBUsed_OneWeekAgo + '</td> 
                <td>' + PercentageOfTotalGBUsedChange + '</td> 
                <td style="color:' + CASE WHEN CAST(TotalDBSizeChange AS FLOAT) > 0 THEN 'red' ELSE 'green' END + ';">' + DatabaseSizeStatus + '</td> <!-- New column data with color formatting -->
...
Burra.Sandhya@cognizant.com;PydaVenkata.SrihimaVishnuSeshasai@cognizant.com;EDMDBA@cognizant.com',
		  ...
		  pydavenkata.srihimavishnuseshasai@cognizant.com',
