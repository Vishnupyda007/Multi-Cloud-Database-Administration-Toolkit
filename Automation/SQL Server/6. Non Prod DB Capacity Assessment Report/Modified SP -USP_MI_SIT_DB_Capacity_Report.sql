...
/****** Object:  StoredProcedure [dbo].[USP_MI_SIT_DB_Capacity_Report]    Script Date: 2/11/2025 10:33:15 AM ******/
SET ANSI_NULLS ON
GO
...
------ ============================================= ***/    
ALTER PROCEDURE [dbo].[USP_MI_SIT_DB_Capacity_Report] with encryption
AS
BEGIN
    CREATE TABLE #temp(
...
        TotalLogFileSizeGB NVARCHAR(50)
    ...
    FROM (
        ...
        FROM MI_SIT_DB_Capacity_Report with(nolock)
    ) AS sub
    WHERE RowNum = 1;
    ...
    BEGIN
        SET @serverName = CASE 
            ...
            when @server = 'ctsinazqaedssqlmi.inso13dfbbbfa2150.database.windows.net' THEN 'EDS QA'
        END;
        ...
        BEGIN
            ...
            WHERE ServerName = @server ORDER BY DBName, FileId ASC;
    ...
    BEGIN  
        ...
        <br>Please find the below MI SIT Servers BI Weekly DB Capacity Report.<br>
  ...
  --      @recipients ='RaviShankar.C@cognizant.com;Janani.M2dcbb3@cognizant.com;Badam.Navya@cognizant.com; Bindu.Raavi@cognizant.com;pradheep.kumartk@cognizant.com; megha.singhal@cognizant.com;
        ...
        @subject = 'Daily MI SIT DB Capacity Report';    
