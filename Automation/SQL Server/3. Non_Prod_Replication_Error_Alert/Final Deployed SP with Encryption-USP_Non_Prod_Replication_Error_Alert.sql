...
/****** Object:  StoredProcedure [dbo].[USP_Non_Prod_Replication_Error_Alert]    Script Date: 2/12/2025 12:09:33 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[USP_Non_Prod_Replication_Error_Alert]        
--SET ANSI_NULLS ON        
--GO        
--SET QUOTED_IDENTIFIER ON        
--GO        
...
------ ============================================= ***/    
ALTER PROCEDURE [dbo].[USP_Non_Prod_Replication_Error_Alert]  
WITH ENCRYPTION
...
BEGIN     
    ...
    INSERT INTO #temp   
    SELECT * FROM [DBAdmin].[dbo].[Non_Prod_Replication_Error_Alert_Mail] with(nolock);  
 ...
 --select * from #temp with(nolock)  
    ...
    BEGIN  
        ...
        FROM   
            (SELECT DISTINCT   
                ...
                time   
            FROM #temp   
            WHERE publisher = 'CTSC01048403401\CRSSIT') AS subquery  
        ORDER BY   
            time DESC,   
            publisher_db DESC   
    ...
    BEGIN  
        ...
        FROM   
            (SELECT DISTINCT   
                ...
                time   
            FROM #temp   
            WHERE publisher = 'CTSC01048405301\CRSUAT') AS subquery  
        ORDER BY   
            time DESC   
        FOR XML PATH('tr'), TYPE       
        ) AS nvarchar(max)) +    
    ...
    --IF EXISTS (SELECT 1 FROM #temp WHERE publisher = 'ctsazsimisgntm1.inso13dfbbbfa2150.database.windows.net')  
    --BEGIN  
    --    SET @hasData = 1;  
    --    SET @body_HTML = @body_HTML +     
    ...
    --            time   
    --        FROM #temp   
    --        WHERE publisher = 'ctsazsimisgntm1.inso13dfbbbfa2150.database.windows.net') AS subquery  
    --    ORDER BY   
    --        time DESC   
    --    FOR XML PATH('tr'), TYPE       
    --    ) AS nvarchar(max)) +    
    ...
    BEGIN  
        ...
        <br>We have detected replication error(s) in CRSSIT/CRSUAT related to below mentioned publication(s).    
  ...
  N'<style>    
        ...
        @subject = 'Action Required: Replication Error Detected in CRSSIT/CRSUAT';    
    END  
