...
/****** Object:  StoredProcedure [dbo].[USP_PAAS_DR_Replication_Error_Alert]    Script Date: 2/12/2025 12:09:33 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[USP_PAAS_DR_Replication_Error_Alert]        
--SET ANSI_NULLS ON        
--GO        
--SET QUOTED_IDENTIFIER ON        
--GO        
...
------ ============================================= ***/    
CREATE PROCEDURE [dbo].[USP_PAAS_DR_Replication_Error_Alert]  
WITH ENCRYPTION
...
BEGIN     
    ...
    INSERT INTO #temp   
    SELECT * FROM [1CDBAMonitoring].[dbo].[PAAS_DR_Replication_Error_Alert_Mail] with(nolock);  
 ...
 --select * from #temp with(nolock)  
    ...
    IF EXISTS (SELECT 1 FROM #temp WHERE publisher = 'ctsazdrmibccrs.inso1a101c37461fa.database.windows.net')  
    BEGIN  
        SET @hasData = 1;  
        SET @body_HTML = @body_HTML +     
        N'<H2 align = "Left"><font face="Lucida Bright" color="blue" size = "2.5">PAAS DR Transactional Replication Error Status:</font></H2>' +    
        N'<table border="1">' +    
        ...
        FROM   
            (SELECT DISTINCT   
                ...
                time   
            FROM #temp   
            WHERE publisher = 'ctsazdrmibccrs.inso1a101c37461fa.database.windows.net') AS subquery  
        ORDER BY   
            time DESC,   
            publisher_db DESC   
  ...
  N'<style>    
        ...
        @subject = 'Action Required: Replication Error Detected in PAAS DR';    
    END  
