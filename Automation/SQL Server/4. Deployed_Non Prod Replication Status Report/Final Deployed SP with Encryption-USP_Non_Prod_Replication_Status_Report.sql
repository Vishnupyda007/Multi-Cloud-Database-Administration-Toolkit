...
/****** Object:  StoredProcedure [dbo].[USP_Non_Prod_Replication_Status_Report]    Script Date: 2/12/2025 12:10:37 PM ******/
SET ANSI_NULLS ON
GO
...
-- Create date: <9:40PM,12 Dec,2024>      
...
BEGIN   
    ...
    INSERT INTO #tempo 
    SELECT * 
    FROM DBAdmin.dbo.Non_Prod_Replication_Status_Report;  

    DECLARE @body_HTML nvarchar(max) = N'';  
    DECLARE @hasData BIT = 0;    

    IF EXISTS (SELECT 1 FROM #tempo WHERE publisher = 'CTSC01048403401\CRSSIT')  
    BEGIN  
        SET @hasData = 1;  
        SET @body_HTML = @body_HTML + 
            ...
            CAST((SELECT td = Publication, '',  
                  ...
                  FROM #tempo 
                  WHERE publisher = 'CTSC01048403401\CRSSIT' 
                  ORDER BY latency DESC
                  FOR XML PATH('tr'), TYPE) AS NVARCHAR(MAX)) +  
            N'</font></table>';
    END;  

    IF EXISTS (SELECT 1 FROM #tempo WHERE publisher = 'CTSC01048405301\CRSUAT')  
    BEGIN  
        SET @hasData = 1;  
        SET @body_HTML = @body_HTML +
            ...
            CAST((SELECT td = Publication, '',  
                         ...
                         td = CAST(last_distsync AS VARCHAR(100)), ''  
                  FROM #tempo 
                  WHERE publisher = 'CTSC01048405301\CRSUAT' 
                  ORDER BY latency DESC
                  FOR XML PATH('tr'), TYPE) AS NVARCHAR(MAX)) +  
            N'</font></table>';  
    END; 
    ...
    BEGIN  
        ...
        EXEC msdb.dbo.sp_send_dbmail   
            @subject = 'Non Prod Replication Status Report - CRSSIT and CRSUAT',  
            @profile_name = 'CRS',  
            @recipients = 'vimalraj.s3@cognizant.com;bindu.raavi@cognizant.com;sneha.s5@cognizant.com;janani.m2dcbb3@cognizant.com;burra.sandhya@cognizant.com',
