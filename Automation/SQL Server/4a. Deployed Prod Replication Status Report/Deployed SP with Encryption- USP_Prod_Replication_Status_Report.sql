...
/****** Object:  StoredProcedure [dbo].[USP_Prod_Replication_Status_Report]    Script Date: 2/12/2025 12:10:37 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[USP_Prod_Replication_Status_Report]      
--SET ANSI_NULLS ON      
--GO      
--SET QUOTED_IDENTIFIER ON      
--GO      
...
ALTER PROCEDURE [dbo].[USP_Prod_Replication_Status_Report]   
WITH ENCRYPTION
...
BEGIN   
    ...
    INSERT INTO #tempo 
    SELECT * 
    FROM [1CDBAMonitoring].dbo.Prod_Replication_Status_Report with(nolock);  

    DECLARE @body_HTML nvarchar(max) = N'';  
    DECLARE @hasData BIT = 0;    

    IF EXISTS (SELECT 1 FROM #tempo WHERE publisher = 'ctsazsimicrsmas.inso1a101c37461fa.database.windows.net')  
    BEGIN  
        SET @hasData = 1;  
        SET @body_HTML = @body_HTML + 
            N'<H2 align="Left"><font face="Lucida Bright" color="blue" size="2.5">PAAS Master Replication Status:</font></H2>' +  
            N'<table border="1">' +  
            ...
            CAST((SELECT td = Publication, '',  
                  ...
                  FROM #tempo 
                  WHERE publisher = 'ctsazsimicrsmas.inso1a101c37461fa.database.windows.net' 
                  ORDER BY latency DESC
                  FOR XML PATH('tr'), TYPE) AS NVARCHAR(MAX)) +  
            N'</font></table>';
    END;  

    IF EXISTS (SELECT 1 FROM #tempo WHERE publisher ='ctsinazprdedssqlmi.inso1a101c37461fa.database.windows.net')  
    BEGIN  
        SET @hasData = 1;  
        SET @body_HTML = @body_HTML +
            N'<br> <br>
			<H2 align="Left"><font face="Lucida Bright" color="blue" size="2.5">EDS Prod Replication Status:</font></H2>' +  
            N'<table border="2">' +  
            ...
            CAST((SELECT td = Publication, '',  
                         ...
                         td = CAST(last_distsync AS VARCHAR(100)), ''  
                  FROM #tempo 
                  WHERE publisher = 'ctsinazprdedssqlmi.inso1a101c37461fa.database.windows.net' 
                  ORDER BY latency DESC
                  FOR XML PATH('tr'), TYPE) AS NVARCHAR(MAX)) +  
            N'</font></table>';  
    END; 
    ...
    BEGIN  
        ...
        EXEC msdb.dbo.sp_send_dbmail   
            @subject = 'Replication Status Report',  
