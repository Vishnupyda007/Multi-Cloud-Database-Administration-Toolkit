...
/****** Object:  StoredProcedure [dbo].[USP_EDS_Prod_Long_Running_Sessions_Alert]    Script Date: 2/25/2025 9:14:06 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[USP_EDS_Prod_Long_Running_Sessions_Alert]       
--SET ANSI_NULLS ON        
--GO        
--SET QUOTED_IDENTIFIER ON        
--GO        
...
--- EXEC [USP_EDS_Prod_Long_Running_Sessions_Alert]   
--
------ ============================================= ***/ 
ALTER PROCEDURE [dbo].[USP_EDS_Prod_Long_Running_Sessions_Alert]
WITH ENCRYPTION
AS
BEGIN
    CREATE TABLE #temp
	...
	[query_runtime_ddhhmmss] > '0:01:00:00';
    ...
    --    SELECT @servername = Servername FROM @servers WHERE ID = @i;
        ...
        IF EXISTS (SELECT 1 FROM #temp WHERE Servername ='ctsinazprdedssqlmi.inso1a101c37461fa.database.windows.net')
        BEGIN
            SET @hasData = 1;

            SET @body_HTML1 = @body_HTML1 +
            N'<H2 align="Left"><font face="Lucida Bright" color="blue" size="2.5">EDS Prod Server Long Running Session(s):</font></H2>' +
            N'<table border="1">' +
	    ...
	    N'<th style="background:#87ceeb">Physical Reads</th>' +
	    N'<th style="background:#87ceeb">Writes</th>' +
	    N'<th style="background:#87ceeb">Logical Reads</th>' +
    ...
    BEGIN
        ...
        N'<p>
        Hi EDS DBA Team,
        <br>We have observed Long Running Session(s) in EDS Prod MI server.Immediate action is necessary to prevent potential system disruptions and ensure continuous performance.<br>
...
select @body_HTML1 as Body_HTML, @body_HTML2 as Body_HTML
        ...
        EXEC msdb.dbo.sp_send_dbmail
		 ...
		 @recipients='EDMDBA@cognizant.com;CRSDBASUPPORT@cognizant.com',
            @subject = 'Critical Alert : EDS Prod Server || Detected Long Running Session(s)';
    END
