...
/****** Object:  StoredProcedure [dbo].[Testing_Views_Definition]    Script Date: 7/4/2025 6:26:18 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[Testing_Views_Definition] with Encryption
...
SELECT @sql = 'bcp "SELECT FormattedView FROM [1CDBAMonitoring].[dbo].ViewsDefinition" queryout "E:\Test.txt" -w -t, -T -S CTSINAZSIPDDB06' 
--'bcp "SELECT ''--createscript--'' UNION ALL SELECT CONVERT(NVARCHAR(MAX), [createscript]) FROM DBAdmin..DRscripts_MI_Users_Roles WHERE CONVERT(DATE, DateT) = CONVERT(DATE, GETDATE()) ORDER BY DateT, Servername, Database_Name, name" queryout \\CTSC00969373301\Backup\temp\test2\CRS_DB_Users_Create_Script.txt -w -t, -T -S' + @@servername;
...
  @recipients= 'VijaiAnand.PV@cognizant.com;KiranKumar.Gannavaram@cognizant.com;Ashwathi.K@cognizant.com;BalaKrishna.Mannepalli@cognizant.com;2275509@cognizant.com',
  @subject = 'ViewDDLs Test',
 
