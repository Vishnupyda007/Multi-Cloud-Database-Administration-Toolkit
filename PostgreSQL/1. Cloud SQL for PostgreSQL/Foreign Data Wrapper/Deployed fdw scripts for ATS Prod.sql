...
create SERVER atseds_db_server
    FOREIGN DATA WRAPPER postgres_fdw
    OPTIONS (host '10.80.220.18', dbname 'ent_data_store', port '5432');


	ALTER SERVER atseds_db_server
    OPTIONS (SET host '10.80.220.18', dbname 'ent_data_store', port '5432');

create USER MAPPING FOR "OneC_4681"
  SERVER atseds_db_server
  OPTIONS (user 'fdw_dbuser',  password 'dBuserFDW@26');

create USER MAPPING FOR postgres
  SERVER atseds_db_server
  OPTIONS (user 'fdw_dbuser', password 'dBuserFDW@26');

  create USER MAPPING FOR "ats-4681-prod-sa@cb0228600a-citprod-gc.iam"
  SERVER atseds_db_server
  OPTIONS (user 'fdw_dbuser', password 'dBuserFDW@26');

--Map devops login too
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format(
      'IMPORT FOREIGN SCHEMA %I LIMIT TO (%s) FROM SERVER atseds_db_server INTO ent_data_store_fdw',
      v_schema, views_csv
    );
  END LOOP;
...
BEGIN
  ...
  -- ------------------------------------------------
  EXECUTE format(
    'IMPORT FOREIGN SCHEMA %I LIMIT TO (%I) FROM SERVER atseds_db_server INTO %I',
...
ALTER SERVER atseds_db_server OPTIONS (
...
SELECT "DBAdmin".grant_edsview_access_func('public.hcm_ps_project_view', '1CDBA_2275509');
