    ...
    ELSE
      ...
      sql_tables := $q$
        ...
        WHERE c.relkind IN ('r','p','m') -- heap, partitioned table, matview
          AND n.nspname NOT IN ('pg_catalog','information_schema','pg_toast','google_vacuum_mgmt','anon') 
...
SELECT pg_get_functiondef('maintenance.usp_update_stats_all_user_dbs_proc'::regproc);




CALL maintenance.usp_update_stats_all_user_dbs_proc(
  '10.75.154.5', 5432, 'postgres', 'DBMaintenanceUser', 'DBmaintenance@26',
  NULL, ARRAY['postgres','template0','template1','cloudsqladmin'],
  NULL, NULL,
  true,  -- false means whole DB; true per-table
  false,  -- ANALYZE (vs VACUUM (ANALYZE))
  0
);
...
SELECT cron.schedule(
    'maintenance_job_analyze_dbs', -- A unique name for your job
    '0 21 15 1 *',                 -- The schedule: 9:00 PM on Jan 15
    $$
    CALL maintenance.usp_update_stats_all_user_dbs_proc(
      '10.75.154.5', 5432, 'postgres', 'DBMaintenanceUser', 'DBmaintenance@26',
      NULL, ARRAY['postgres','template0','template1','cloudsqladmin'],
