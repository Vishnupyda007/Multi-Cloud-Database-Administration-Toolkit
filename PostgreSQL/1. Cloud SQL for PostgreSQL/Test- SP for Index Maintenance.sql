...
BEGIN
  ...
  -------------------------------------------------------------------------
  -- Candidate SQL (Estimator — from community bloat query, no extensions)
  -- See PostgreSQL wiki "Show database bloat" reference; we simplify to
  -- compute index bloat percentage (ibloat) and size. [1](https://wiki.postgresql.org/wiki/Show_database_bloat)
  -------------------------------------------------------------------------
    ...
    WITH bs AS (SELECT current_setting('block_size')::int AS bs),
         ...
         est AS (
           SELECT schema, table_name, index_name,
                  ipages, ituples,
                  -- very rough expected pages for btree:
  ...
  -- We measure free_percent on the *index* to estimate bloat directly. [2](https://www.postgresql.org/docs/current/pgstattuple.html)
  -------------------------------------------------------------------------
  ...
  FOR db_row IN
    ...
    WHERE (p_include_dbs IS NULL OR datname = ANY(p_include_dbs))
      AND (p_exclude_dbs IS NULL OR NOT (datname = ANY(p_exclude_dbs)))
  LOOP
    conn_name := 'reindex_' || db_row.datname || '_' || replace(md5(random()::text), '-', '');
    ...
    IF p_use_pgstattuple THEN
      ...
      LOOP
        ...
        VALUES (db_row.datname,
                ...
                'REINDEX_CANDIDATE',
                now(), now(), true,
                format('mode=pgstattuple bloat_pct=%.2f size=%s',
                  r_idx.bloat_pct,
        ...
        IF NOT p_dry_run THEN
          BEGIN
            ...
            -- doc: CONCURRENTLY uses lower lock; rebuilds index safely. [4](https://www.postgresql.org/docs/17//sql-reindex.html)[5](https://postgrespro.com/docs/postgresql/current/routine-reindex)
            INSERT INTO maintenance.stats_update_run_log(dbname, target, action, started_at, finished_at, success, detail)
            VALUES (db_row.datname,
                    format('%s.%s', r_idx.schema, r_idx.index_name),
                    'REINDEX',
                    now(), now(), true,
                    format('reindexed concurrently (pgstattuple bloat=%.2f%%)', r_idx.bloat_pct));
          EXCEPTION WHEN others THEN
            GET STACKED DIAGNOSTICS v_sqlstate = RETURNED_SQLSTATE, v_msg = MESSAGE_TEXT;
            INSERT INTO maintenance.stats_update_run_log(dbname, target, action, started_at, finished_at, success, detail)
            VALUES (db_row.datname,
    ...
    ELSE
      -- Estimator mode (no extension). Based on wiki bloat approach (approx). [1](https://wiki.postgresql.org/wiki/Show_database_bloat)
      FOR r_idx IN
      ...
      LOOP
        ...
        VALUES (db_row.datname,
                ...
                'REINDEX_CANDIDATE',
                now(), now(), true,
                format('mode=estimator bloat_pct=%.2f size=%s',
                  r_idx.bloat_pct,
                  pg_size_pretty(r_idx.idx_bytes)));
        ...
        IF NOT p_dry_run THEN
          BEGIN
            ...
            VALUES (db_row.datname,
                    ...
                    'REINDEX',
                    now(), now(), true,
                    format('reindexed concurrently (est bloat=%.2f%%)', r_idx.bloat_pct));
          EXCEPTION WHEN others THEN
            GET STACKED DIAGNOSTICS v_sqlstate = RETURNED_SQLSTATE, v_msg = MESSAGE_TEXT;
            INSERT INTO maintenance.stats_update_run_log(dbname, target, action, started_at, finished_at, success, detail)
            VALUES (db_row.datname,
