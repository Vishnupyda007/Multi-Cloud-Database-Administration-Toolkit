...
SELECT cron.schedule(
  ...
  '15 0 * * *',
  $$SELECT public.reconcile_manual_runs(120);$$
);
...
-- SELECT cron.schedule('cleanup-oneoff-cron-jobs','5 0 * * *',$$SELECT public.cleanup_oneoff_cron_jobs();$$);
...
-- SELECT cron.schedule('purge-manual-logs','0 1 * * 0',$$SELECT public.purge_manual_logs(28);$$);
...
-- SELECT public.run_cron_job_manually('cleanup-oneoff-cron-jobs') AS one_off_jobid;
