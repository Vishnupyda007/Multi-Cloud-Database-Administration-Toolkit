...
CREATE OR REPLACE FUNCTION maintenance.cleanup_maintenancestats_Reindex_update_run_log_dblink()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
        ...
        BEGIN
            ...
            FROM dblink(
                ...
                SELECT EXISTS (
                    ...
                    WHERE table_schema = 'maintenance'
                      AND table_name   = 'stats_update_run_log'
                ) AS exists_flag
                $q$
            ...
            IF tbl_exists THEN
                PERFORM dblink_exec(
                    conn_name,
                    $del$
                    DELETE FROM maintenance.stats_update_run_log
...
SELECT cron.schedule(
  'cleanup_maintenancestats_Reindex_update_run_log',
  '30 3 * * *',
  $$SELECT maintenance.cleanup_maintenancestats_Reindex_update_run_log_dblink();$$
);
