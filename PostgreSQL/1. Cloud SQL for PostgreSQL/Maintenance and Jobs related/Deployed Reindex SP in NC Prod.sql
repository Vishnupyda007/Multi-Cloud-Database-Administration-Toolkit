...
BEGIN
    ...
    -- FULLY QUALIFIED to "DBAdmin".pgstattuple with explicit ::regclass cast
    ----------------------------------------------------------------------
    sql_candidates_pgstat := $PGS$
        SELECT n.nspname AS schema,
               c.relname  AS index_name,
               ct.relname AS table_name,
               ("DBAdmin".pgstattuple(c.oid::regclass)).free_percent::numeric(10,2) AS bloat_pct,
    ...
    LOOP
        ...
        IF p_use_pgstattuple THEN
            -- Ensure extension exists in "DBAdmin"; if not, log and fallback to estimator.
            BEGIN
...
GRANT EXECUTE ON PROCEDURE maintenance.usp_reindex_bloated_indexes_proc(text, integer, text, text, text, text[], text[], text, text, numeric, boolean, boolean, integer) TO "DBMaintenanceUser";
...
grant al on table maintenance.stats_update_run_log to "DBMaintenanceUser";
...
true,    -- true means p_dry_run only and false means real run
0
);
...
SELECT cron.alter_job(
    job_id := 5,  -- The name of the job you are targeting
